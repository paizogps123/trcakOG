...
function getmycuurentpic()
{
	...
	$('#txt_b_address').val(placesMarkerData.ambulance_addr);
	closeambulancepic();
}
...
function fnloadgrid(type)
{
	...
	var chartData = [ {
		...
		/*
		var chart = AmCharts.makeChart( "chartsensor", {
		  "theme": "light",
		  "type": "serial",
	...
	$.ajax({
        type: "GET",
        datatype: "json",
        url: "func/dashboard.php?cmd=liveroute&condition="+flag+"&type="+type,
        cache: false,
        success: function(c) {
        //console.log(c);
		
		$('#eventcount').html(c.todayevents);
	    ...
	    $("#spantotalvehicle").text(vonline+voffline);
        ...
        $('#grid_container div:not(.ui-jqgrid-titlebar)').width("100%");    
            ...
            chart1.dataProvider[0].color = "#99B958";
            chart1.dataProvider[1].color = "#F5456D";
            chart1.validateData();
...
function hotspotin(savecancel)
{
	...
	if(hotspotsavename=="")
	 ...
	 var dat = {cmd: "save_hotspot",hotspot_code: vboarding.hotspotcode,hotspot_name: hotspotsavename,zonedetail:vt10data };
		$.ajax({
                type: "POST",
                url: "func/fn_boarding.php",
                data: dat,
...
$(document).ready(function() {
	...
	document.getElementById( 'showRightD' ).onclick = function() {fnmenuright(this);	};
	...
	$(document).ready(function(){
	    $("#tglDBMV").toggle(
	        ...
	        $("#searchobjectsetting").bind("keyup", function(d) {
	        	jQuery("#settings_main_object_list_grid").jqGrid().setGridParam({url :  "func/fn_settings.objects.php?cmd=load_object_list&nameimei="+this.value}).trigger("reloadGrid");
	        }),
	     	$("#searcheventsetting").bind("keyup", function(d) {
	    		jQuery("#settings_main_events_event_list_grid").jqGrid().setGridParam({url :  "func/fn_settings.events.php?cmd=load_event_list&nameimei="+this.value}).trigger("reloadGrid");
	    	})
...
function replacecmd()
{
	if(nowcmd.length>0 && $('#cmd_object_listnew').val()!="Select" && $('#cmd_devicetype  option:selected').val()=="Play FM100")
	{
...
var clasch=document.getElementById("ping").className;
    if(clasch=='pincl'){
      document.getElementById("map").className = "block width65 leaflet-container leaflet-touch leaflet-fade-anim leaflet-grab leaflet-touch-drag leaflet-touch-zoom";
      map.invalidateSize(!0)            
      document.getElementById("ping").className = "pingch";
      document.getElementById("chpinim").src="img/unpin.png";
      document.getElementById("bottom_panel").style.width = "64%";      
      ...
      document.getElementById("myDropdown").style.display = "block";
        ...
        var drop1=document.getElementById("myDropdown2").style.display;
        var drop2=document.getElementById("myDropdown3").style.display;
        var drop3=document.getElementById("myDropdown4").style.display ;
        ...
        // alert(document.getElementById("myDropdown").style.display);
        if(drop1!="block" || drop2!="block" || drop3!="block" ){
            alert('hai');
            document.getElementById("myDropdown").style.display = "block";
