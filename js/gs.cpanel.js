...
function switchCPManager(manager_id)
{
	...
	$('#cpanel_supplier_list_grid').setGridParam({url:'func/fn_cpanel.objects.php?cmd=load_object_list&manager_id=' + cpValues['manager_id']});
	$('#cpanel_supplier_list_grid').trigger("reloadGrid");
...
function setUserAccessSettings(){
	var data = {
		cmd: 'load_user_access_settings'
	};
	...
	$.ajax({
		...
		success: function(result)
		{
			cpValues['accessSettings']=result;
		}
	});	
}