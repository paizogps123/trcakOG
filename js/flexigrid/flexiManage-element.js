    ...
    $.addFlexPanel = function (t, p) {
        ...
        var g = {
            ...
            addSerachBarModel: function (item) {//search bar
                ...
                var cont = '';
                cont += '<input name="' + name + '" data-placeholder="' + value + '" placeholder="' + value + '" autocomplete="off" class="search-input search-text ">';
                ...
                $(mod_2).append(cont);
                $(mod_1).append(mod_2);
                $(mod_1).append('<span class="y-btn y-btn-default y-btn-submit"><i class="icon fa fa-search"></i></span>');
            ...
            addComBoboxModel: function (params) {//selection box
                ...
                combobox.className = combobox.className + ' comBobox';
                var multiple = params.combox.multiple;//Multiple choice
                var defaultValue = params.combox.defaultValue;//default value
                var checkValueArr = params.combox.checkValue ? params.combox.checkValue.split(',') : [];//Check value
                ...
                if (items == null || items == '') {
                    ...
                    $(mod_li).append('<i class="icon bg-icon bi bi-chevron-down"></i>');
                    ...
                    for (var i = 0; i < items.length; i++) {
                        ...
                        if (params.combox.preicon) {
                            if (i == 0) {
                                //Check or not
                                if (checkValueArr.length > 0 && checkValueArr.contains(item.key)) {
                                    $(mod_li).append('<i class="icon pre-icon cur-timeline-icon checked"></i>');
                                } else {
                                    $(mod_li).append('<i class="icon pre-icon cur-timeline-icon"></i>');
                                }
                            } else {
                                //Check or not
                                if (checkValueArr.length > 0 && checkValueArr.contains(item.key)) {
                                    $(mod_li).append('<i class="icon pre-icon checked"></i>');
                                } else {
                    ...
                    //query box
                    var isSearch = params.combox.search;
                    if (isSearch) {
                        //Input query box
                        mod_input = document.createElement('input');
                        mod_input.className = 'ui-menu-item-search';
                        $(mod_ul).prepend(mod_input);
                        //Bind input events
                        mod_input.addEventListener("keyup", function (e) {
                            $("#ui-menu-item-search-result-ul").remove();
                            var inputVal = $(this).val();
                            if (!inputVal) {
                                return;
                            }
                            var searchItems = g.strToArray(params.combox.option, '|', '&');
                            var searchResult = [];
                            for (var i = 0; i < searchItems.length; i++) {
                                var item = searchItems[i];
                                if (item.value.indexOfNotCase(inputVal) >= 0) {
                            ...
                            // if (searchResult.length > 0) {
                                ...
                                var search_result_ul = document.createElement('ul');
                                search_result_ul.className = 'scroll-list ui-menu-item-search-result-ul';
                                search_result_ul.id = 'ui-menu-item-search-result-ul';
                                $(mod_input).after(search_result_ul);
                                if (searchResult.length > 0) {
                                    for (var i = 0; i < searchResult.length; i++) {
                                        var item = searchResult[i];
                                        var search_result_li = document.createElement('li');
                                        search_result_li.className = 'ui-menu-item-search-result';
                                        if (item.key) {
                                            $(search_result_li).attr('data-index', item.key);
                                        }
                                        $(search_result_li).append('<span class="text" title="' + item.value + '">' + item.value + '</span>');
                                        $(search_result_ul).append(search_result_li);
                                    }
                                }
                                //scroll to top
                                $(mod_ul).scrollTop(0);
                                //Click on the menu to associate
                                $('.ui-menu-item-search-result', search_result_ul).on('click', function () {
                                    var key = $(this).attr('data-index');
                                    $("#ui-menu-item-search-result-ul").remove();
                                    $(mod_input).val("");
                                    $(mod_1).find('.ui-menu-list-box .ui-menu-item[data-index=\'' + key + '\']').trigger("click");
                                });
                            // }
                        });
                    }
                ...
                if (items && items.length > 0 && multiple) {
                    ...
                    if (params.combox.multipleTitle) {
                        ...
                        $(mod_3).append('<div class="multiple-select" style="color: #fff;margin-top:5px;"><div class="ttx-themeBgColor" style="cursor: pointer;text-align: center;margin: 0px auto;">' + title + '</div></div>');
                    }
                }
                $(mod_2).append(mod_3);
                $(mod_1).append(mod_2);
               ...
               if($('.page').length == 1){
                   $('.page').append(mod_1);
               }else if($('#main_content').length == 1){
                 $('#main_content').append(mod_1);
               }else {
                   $('body').append(mod_1);
               }
                ...
                $('.y-btn .label', combobox).on('input propertychange click', function (e) {
                    ...
                    if (maxHeight_ + $(mod_1).height() > getWindowHeight() + getScrollTop()) {
                        //When the width and height of the drop-down exceeds the upper boundary, the height becomes smaller
                        var height = $(mod_1).height();
                        if (multiple) {
                            if (params.combox.multipleTitle) {
                                if (($(mod_1).height() + $('.multiple-select').height() + 10) > comb_top_ - getScrollTop()) {
                                    height = (comb_top_ - $('.multiple-select').height() - getScrollTop() - 20);
                                    $(mod_ul, mod_1).css('height', height + 'px');
                                }
                            } else {
                                if (($(mod_1).height() + 10) > comb_top_ - getScrollTop()) {
                                    height = (comb_top_ - getScrollTop() - 20);
                                    $(mod_ul, mod_1).css('height', height + 'px');
                                }
                            }
                        } else {
                            if ($(mod_1).height() > comb_top_ - getScrollTop()) {
                                height = (comb_top_ - getScrollTop() - 10);
                                $(mod_ul, mod_1).css('height', height + 'px');
                            }
                        }

                        maxHeight_ = comb_top_ - height;
                    }
                    //Judge again fix:#28983
                    comb_top_ = getTop($(combobox).get(0));
                    comb_height_ = $(combobox).height();
                    var maxHeight_Ex = comb_top_ + comb_height_;
                    if (maxHeight_Ex + $(mod_1).height() <= getWindowHeight() + getScrollTop()) {
                        maxHeight_ = maxHeight_Ex;
                    }
                    $(mod_1).css('top', maxHeight_ + 'px'); //CSS has margin plus 10, here subtract 10
                    //$(mod_1).css('top', getTop($(combobox).get(0)) + $(combobox).height() + 'px');
                    if(params.style && params.style.left){
                        $(mod_1).css('left', getLeft($(combobox).get(0)) + params.style.left + 'px');
                    }else{
                        $(mod_1).css('left', getLeft($(combobox).get(0)) + 'px');
                    }
                    if ($(mod_1).width() < $('.item', combobox).width()) {
                        if(params.style && params.style.width){
                            $(mod_1).css('width', params.style.width);
                        }else{
                    ...
                    if (e.type == 'click') {
                        isOut = false;
                        $(mod_1).show();
                        $(this).parent().addClass('buttonShow');
                    }
                    ...
                    $('.ui-menu-item', mod_1).each(function () {
                        ...
                        if (flag) {
                            ...
                            if (params.button) {
                                if (value.toLowerCase() === (mval.toLowerCase())) {
                    ...
                    if ($(mod_1).css('display') == 'none') {
                        ...
                        if (maxHeight_ + $(mod_1).height() > getWindowHeight() + getScrollTop()) {
                            ...
                            if (multiple) {
                                if (params.combox.multipleTitle) {
                                    if (($(mod_1).height() + $('.multiple-select').height() + 10) > comb_top_ - getScrollTop()) {
                                        $(mod_ul, mod_1).css('height', (comb_top_ - $('.multiple-select').height() - getScrollTop() - 20) + 'px');
                                    }
                                } else {
                                    if (($(mod_1).height() + 10) > comb_top_ - getScrollTop()) {
                                        $(mod_ul, mod_1).css('height', (comb_top_ - getScrollTop() - 20) + 'px');
                                    }
                                }
                            } else {
                                if ($(mod_1).height() > comb_top_ - getScrollTop()) {
                                    $(mod_ul, mod_1).css('height', (comb_top_ - getScrollTop() - 10) + 'px');
                                }
                            }
                    ...
                    $('.ui-menu-item', mod_1).on('click', function () {
                        ...
                        if (isHasAll) {//If included select all
                            ...
                            if ($(this).index() == allIndex) {
                                ...
                                var checkedItems = 0;
                                $('.ui-menu-item', mod_1).each(function () {
                                    if ($(this).index() > allIndex && $('.icon', this).hasClass('checked')) {
                                        checkedItems++;
                                    }
                                });
...
//						}
                    ...
                    $('.ui-menu-item', mod_1).on('click', function () {
                        ...
                        isOut = true;
                        $('.btn-group .item .y-btn.buttonShow').removeClass('buttonShow');
                ...
                var checkComboxParam = function () {
                    ...
                    if (!multiple) {
                        ...
                        $('.ui-menu-item', mod_1).each(function () {
                            $(this).attr('data-id', $(combobox).parent().parent().parent().attr('id'));
                            var mval = $.trim($('.text', this).html());
                            if (mval.toLowerCase() == cval.toLowerCase()) {
                                var key = $(this).attr('data-index');
                                var value = $.trim($('.text', this).html());
                                if (params.button) {
                                    $('.y-btn .label', combobox).html(value);
                ...
                if (items && items.length > 0 && !multiple && defaultValue != null && defaultValue !== '') {
                    $(mod_ul).find('li[data-index='+defaultValue+']').click();
                }
                return combobox;
            ...
            addInputModel: function (params) {
                ...
                btn.className = 'y-btn';
                params.bgcolor = params.bgcolor && params.bgcolor !== 'gray' ? params.bgcolor : 'default';
                if (params.bgcolor) {
                ...
                if (params.bgicon) {
                    content += '<i class="icon bg-icon bi bi-chevron-down"></i>';
                }
                $(btn).append(content);
                ...
                if (params.extraClass) {
                    $('.search-input', btn).addClass(params.extraClass);
                }
                $(btn).on('click', function () {
                    if (!$(this).hasClass('buttonShow') && !$(btn).find('input').prop("disabled") && !$(btn).find('input').prop("readonly")) {
                        $(this).addClass('buttonShow');
                    }
                })
                $('body').on('click', function (e) {
                    if ($(e.target).closest('.label.search-input').length <= 0 && $(e.target).closest('.bg-cion.bi-chevron-down').length <= 0) {
                        $(btn).removeClass('buttonShow');
                    }
                });
                return input;
            },
            addButtonsModel: function (params) {//button
                ...
                for (var i = 0; i < params.length; i++) {
                    ...
                    for (var j = 0; j < groups.length; j++) {
                        ...
                        btn.className = 'y-btn';
                        items.bgcolor = items.bgcolor && items.bgcolor !== 'gray' ? items.bgcolor : 'default';
                        //Submit and query buttons are uniformly set to theme colors
                        if (items.pclass && (items.pclass == 'submit' || items.pclass == 'btnSave' || items.pclass == 'save' || item.pclass == 'btnQuery')) {
                            items.bgcolor = 'primary';
                        }
                        if (items.bgcolor) {
            ...
            addTableGroupModel: function (params) {
                ...
                for (var i = 0; i < params.length; i++) {
                    ...
                    if ($(title).attr('tabshide') || title.tabshide) {
                        cont += '<i class="head-control bi bi-chevron-right"></i>';
                    } else {
                        cont += '<i class="head-control current bi bi-chevron-down"></i>';
                    }
                    if (title.display) {
                    ...
                    $(adiv).on('click', function () {
                        var head_control = $(this).find('.head-control');
                        head_control.toggleClass('current');

                        if (head_control.hasClass('bi bi-chevron-down')) {
                            head_control.removeClass('bi bi-chevron-down').addClass('bi bi-chevron-right')
                        } else if (head_control.hasClass('bi bi-chevron-right')) {
                            head_control.removeClass('bi bi-chevron-right').addClass('bi bi-chevron-down');
                        }
            ...
            addCheckBoxGroupModel: function (params) {
                ...
                for (var i = 0; i < params.length; i++) {
                    ...
                    if (!$(title).attr('hide')) {
                        ...
                        if (title.display) {
                            if (title.name) {
                                cont += '<H3 class="h3" name="' + title.name + '">' + title.display + '</H3>';
                            } else {
                                cont += '<H3 class="h3">' + title.display + '</H3>';
                            }
                        } else {
                        ...
                        for (var j = 0; j < page; j++) {
                            ...
                            for (var k = cols.length * j; k < (j + 1) * cols.length && k < maxCount; k++) {
                                ...
                                if (names && names.length > k && names[k] != null && names[k] != '') {
                                    ...
                                    if (langKey && langKey[k]) {
                                        $(pl).attr('data-lang', langKey[k]);
                                    }
                        ...
                        $('.head-control', adiv).on('click', function () {
                            ...
                            if ($(this).hasClass('bi bi-chevron-down')) {
                                $(this).removeClass('bi bi-chevron-down').addClass('bi bi-chevron-right')
                            } else if ($(this).hasClass('bi bi-chevron-right')) {
                                $(this).removeClass('bi bi-chevron-right').addClass('bi bi-chevron-down');
                            }
                    ...
                    if (!$(title).attr('hide')) {
                        ...
                        if (styleId === '4' || styleId === '5') {
                            if (title.pclass && title.pclass.indexOf('active') > -1) {
                                $(part_menu).append('<i class="bi bi-chevron-down bg-icon open-fold reversal"></i>');
                            } else {
                                $(part_menu).append('<i class="bi bi-chevron-down bg-icon open-fold"></i>');
                            }
                        }
        ...
        if (p.InputModel) {
            p.InputModel.extraClass = 'only-input';
            $(t).prepend(g.addInputModel(p.InputModel));
            if (p.InputModel.label) {
                $(t).prepend('<span class="flexi-label">' + p.InputModel.label + '：</span>');
                $(t).addClass('flexi-flex')
            }
            //Set the input box to be just a search box, with no icon on the right and a width of 174
            if (p.InputModel.onlySearch) {
                $(t).find('input').eq(0).addClass('onlySearch');
            }
        }
