    ...
    init: function () {
        ...
        for (E = 1, C = A; E < 7; E++) {
            ...
            for (D = 0; D < 7; D++) {
                ...
                if (_.M == B) {
                    K = true;
                    if (_.compareWith($sdt, "d") == 0) I = "Wselday"; else if (_.compareWith($tdt, "d") == 0) I = "Wtoday"; else I = ($dp.highLineWeekDay && (0 == (G + D) % 7 || 6 == (G + D) % 7) ? "Wwday" : "Wday");
        ...
        if(this.dateFmt === 'yyyy'){
            $d.todayI.value = $lang.curYearStr;
        }
        if(this.dateFmt === 'yyyy-MM'){
            $d.todayI.value = $lang.curMonStr;
        }
        if(this.dateFmt === 'HH:mm:ss')
            $d.todayI.value = $lang.nowStr;{
        }
        ...
        if ($dp.eCont || !($dp.isShowClear || $dp.isShowToday || $dp.isShowOK)) hide($d.bDiv); else show($d.bDiv)
    }, mark: function (B, D) {
        ...
        _ = _ || D + D;
        if ("yMdHms".indexOf(D) > -1 && D != "y" && !$dp.has[D]) if ("Hms".indexOf(D) > -1) A = 0; else A = 1;
        var B = [];
        ...
        for (var C = 0; C < _.length; C++) {
            ...
            if (_[C].contentWindow == window && A) {
                ...
                if ($d.offsetWidth==1){
                    _[C].style.width = ($d.scrollWidth + 10) + "px";
                }
        ...
        if (E > $) E = $; else if (_ == "m" || _ == "s") C = [-60, -30, 0, 30, 60, -15, 15, -45, 45]; else for (H = 0; H < $; H++) C[H] = $dt[_] - 2 + H;
        for (H = G = 0; H < E; H++) {
...
function c(p, pv) {
    if (p == "M") pv = makeInRange(pv, 1, 12); else if (p == "H") pv = makeInRange(pv, 0, 23); else if ("ms".indexOf(p) >= 0) pv = makeInRange(pv, 0, 59);
    if ($sdt[p] != pv && !callFunc(p + "changing")) {
        var func = "sv(\"" + p + "\"," + pv + ")", rv = $c.checkRange();
        if (rv == 0) eval(func); else if (rv < 0) _setFrom($c.minDate); else if (rv > 0) _setFrom($c.maxDate);
        $d.okI.disabled = !$c.checkValid($sdt);
...
function _focus($) {
    ...
    if (!_) return;
    $c.currFocus = this;
    if (_ == "y") this.className = "yminputfocus"; else if (_ == "M") {
    ...
    if ($dp.enableKeyboard && isShow) {
        ...
        if (Q >= 37 && Q <= 40) {
            ...
            if ($c.currFocus == $dp.el || $c.currFocus == $d.okI) if ($dp.has.d) {
                U = "d";
                if (Q == 38) $dt[U] -= 7; else if (Q == 39) $dt[U] += 1; else if (Q == 37) $dt[U] -= 1; else $dt[U] += 7;
                $dt.refresh();
                c("y", $dt["y"]);
                c("M", $dt["M"]);
                c("d", $dt[U]);
            ...
            if (U) {
                if (Q == 38 || Q == 39) $dt[U] += 1; else $dt[U] -= 1;
                $dt.refresh();
            ...
            var D = H.nextCtrl;
            for (var R = 0; R < $dp.focusArr.length; R++) if (D.disabled == true || D.offsetHeight == 0) D = D.nextCtrl; else break;
            if ($c.currFocus != D) {
                $c.currFocus = D;
                D.focus()
            }
            ...
            if ($c.currFocus.type == "button") $c.currFocus.click(); else $c.pickDate();
            $c.currFocus = $dp.el
        }
