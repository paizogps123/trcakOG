...
(function () {
    ...
    function W(F) {
        if (F.getBoundingClientRect)
            ...
            var A = {
                ...
                }, E = false, H = null, _ = F.offsetTop, G = F.offsetLeft, D = F.offsetWidth, B = F.offsetHeight,
                C = F.offsetParent;
            if (C != F)
                while (C) {
                    G += C.offsetLeft;
        ...
        function D() {
            if (I) {
                ...
                while (func != null) {
                    ...
                    if ($ && ($.originalEvent + "").indexOf("Event") >= 0)
                        return $;
                    func = func.caller
                }
                return null
    ...
    function K(H, $) {
        ...
        if (!$dp.dd
            ...
            if (_.$dpPath == "")
                F(A);
            var B = "<iframe style=\"width:1px;height:225px\" src=\""
                + _.$dpPath
        ...
        function C() {
            ...
            var $dpTitle = $dp.dd.getElementsByTagName("iframe")[0].contentWindow.document.getElementById("dpTitle")
            if($dpTitle && $dpTitle.nextElementSibling){
                var nextElementSibling = $dpTitle.nextElementSibling;
                // nextElementSibling.style.width = 'calc(100% - 6px)';
                // nextElementSibling.style.height = 'calc(100% - 56px)';
                nextElementSibling.style.width = '180px';
                nextElementSibling.style.height = '142px';
            }
        }
    }
})()
