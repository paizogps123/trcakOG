			...
			if(allCells.length > 0)
			{
				...
				allCells.each(function(index, option)
				{
					...
					if(regExp.test($(option).text()))
					{
						if(vfoption=="")
						{
							vfoption=option.value;
							$(searchArea).val(vfoption);
						}
						$(option).show();
					}
					else
...
{
    if (document.URL.indexOf("func/fn_object.follow.php") != -1 || document.URL.indexOf("LiveTrack.php") != -1 || document.URL.indexOf("index.php") != -1)
    {
	return true;
    }
    else
...
function transformToObjectData(data)
{
	...
	result['lastevent']=data['lastevent'];
                ...
                for (var imei in settingsObjectData)
                {
                        ...
                        if (object.group_id == group_id)
                        {
                                if (object.active == "true")
                                {
                                        ...
                                        {
                                                items_arr.push({ value: imei, name: object.name,vechiletype:object.vehicle_type })      
                                        }
                                        else
                                        {
...
function loadLanguageAPI(response)
{
...
// END OTHER FUNCTIONS
// #################################################
function getGroupsreportArray(list,group)
{        
		var select=list;
        var groups_arr = new Array();
        ...
        {
        		var group1=group[group_id].gid;
                var items_arr = new Array();
                
                for (var imei in select)
                {
                        var object = select[imei];
                        
                        if (object.group_id == group1)
                        {
                                if (object.active == "A")
                                {
                                    items_arr.push({ value: object.rid, name: la[object.report_lng]})       
                                       
                                }
                        }
                }
                
                var group_name = group[group_id].group_name;
                
                items_arr = sortArrayByElement(items_arr, 'name');
                
                groups_arr.push({ name: group_name, id: group1, items: items_arr });
        }
...
function checkBoxEnable(v){
	if(document.getElementById(v).checked == true){
		document.getElementById(v+'_txt').disabled = false;
	}else{
		document.getElementById(v+'_txt').disabled = true;
		document.getElementById(v+'_txt').value = '';
	}
}
