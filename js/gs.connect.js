...
function connectLogin()
{
	...
	jQuery.ajax({
		...
		success: function(result)
		{
			...
			else if(result=='MANAGER_ACCOUNT')
			{
				// window.open ('http://track.playgps.co.in/beta/','_self',false);
				window.open ('cpanel.php','_self',false);
			}
			else if (result == 'ERROR_USERNAME_PASSWORD_INCORRECT')
			{
				$('#dialog').removeClass('dialog-effect-out').removeClass('shakeit');
