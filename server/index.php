...
// fclose($myfile);
	...
	if(@$_POST['vehicle'] == '' || @$_POST['lat'] == ''  || @$_POST['lng'] == ''  || @$_POST['datetime'] == '' )
	{
		$result=array();
		$result["type"]="E";
	...
	if(@$_POST['method'] == 'api_insert')
	{
		...
		$loc['imei'] = strtoupper(@$_POST['vehicle']);
		$loc['lat'] = @$_POST['lat'];
		$loc['lng'] = @$_POST['lng'];
		...
		$loc['params'] = stripslashes(@$_POST['sensors']);
		$loc['params'] = json_decode($loc['params'],true);
		$loc["params"]["acc"]=$loc["params"]["engine_stats"];
	...
	function insert_db_loc($loc)
	{
		...
		// offline data
		$dt_diff = abs(strtotime($loc['dt_server']) - strtotime($loc['dt_tracker']));
		if($dt_diff > 60)
		{
			return false;
		}
