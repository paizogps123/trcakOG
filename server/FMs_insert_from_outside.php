<?
	...
	if(@$_POST['method'] == 'amwellInsertNew')
	{
		if(@$_POST["event"]=="fueldiscnt")
		{
			$_POST["event"]="";
		}
		...
		$loc['params'] =$_POST['params']."gpsL=".@$_POST['signal_gps']."|";
		...
		if (($loc['lat'] != 0) && ($loc['lat'] != 0))
		{
			...
			if($loc['acc']==1)
			{
			   if(isset($loc['FMS']))
			   {
					if($loc['FMS']=="on")
					{
						$loc["fuelused"] = (getParamValue($loc['params'],"fuelused"));
						$loc["odometer"] = (getParamValue($loc['params'],"odo")); 					
					}
					
				}
			}
			
			if(!isset($loc["fuelused"]))
			{
				$loc["fuelused"] = (getParamValue($loc_prev['params'],"fuelused"));
				$loc["odometer"] = (getParamValue($loc_prev['params'],"odo")); 
			}
	...
	function insert_db_status($loc, $loc_prev)
	{
		...
		if (strtotime($loc['dt_tracker']) >= strtotime($loc_prev['dt_tracker']))
		{
			...
			if ($dt_last_stop >= $dt_last_move && $dt_last_idle>=$dt_last_aconidle)
			{
				$sensor = getSensorFromType($imei, 'acc');
				$acc = $sensor[0]['param'];
				$ac="di1";
				...
				if (isset($params[$acc]))
				{
					if ((($params[$acc] == 1) && ($dt_last_idle <= 0)) && (($params[$ac] == 1) && ($dt_last_aconidle <= 0)))
					{
						$q = "UPDATE gs_objects SET `dt_last_aconidle`='".$loc['dt_server']."' WHERE imei='".$imei."'";
						$r = mysqli_query($ms, $q) or die(mysqli_error($ms));	
					}
					else if ((($params[$acc] == 0) && ($dt_last_idle > 0)) || (($params[$ac] == 0) && ($dt_last_aconidle > 0)))
					{
	...
	function Replacelossdata($loc,$loc_prev,$arr2odo)
	{
			...
			else if($loc_prev["fueltype"]=="FMS")
			*/
		
			{
				$acc = getParamValue($loc['params'],"acc");
				$loc['acc']=$acc;
				if($acc==1)
				{
					$fuelused=getParamValue($loc['params'],"fuelused");
					if($fuelused==0)
					{
						$fuel1prev=(getParamValue($loc_prev['params'],"fuelused"));
						$loc['params']["fuelused"]=$fuel1prev;
						
						$fuel1prev=(getParamValue($loc_prev['params'],"fuel1"));
						$loc['params']["fuel1"]=$fuel1prev;
						
						$loc['fuelused']=$loc_prev['fuelused'];
						$loc['odometer']=$loc_prev['odometer'];
					}
				}
				else 
				{
