<?
	...
	// $myfile = fopen("0ilocktracker.txt", "w");
	// fwrite($myfile,json_encode($_POST));
	// fwrite($myfile, "\n");
	// fclose($myfile);

	if($_POST["imei"]=="88000098")
	{
	$myfile = fopen("0ilocktracker_88000098.txt", "a");
	fwrite($myfile,json_encode($_POST));
	fwrite($myfile, "\n");
	fclose($myfile);
	}
	
	if(isset($_POST['rfid']) and $_POST['rfid']!=''){
	...
	if(@$_POST['method'] == 'Loc' || @$_POST['method'] == 'Alarm' )
	{
		...
		if ($dt != ""){
			//23 09 09 10 56 40
			$loc['dt_tracker'] = "20".substr($dt,0,2)."-".substr($dt,2,2)."-".substr($dt,4,2)." ".substr($dt,6,2).":".substr($dt,8,2).":".substr($dt,10,2);
		} else{
		...
		$loc['type'] = @$_POST['type'];
		$loc['alarm'] = @$_POST['event'];
		//$loc['event'] = @$_POST['event'];
		$loc['digital'] = @$_POST['digital'];
		...
		/*$loc['params'] = "ignition=".@$_POST['ignition']."|"."ac=".@$_POST['ac']."|"."input1=".@$_POST['input1']."|"
		...
		*/
		...
		$strdigkey = str_pad(decbin((int)$digital), 8, '0', STR_PAD_LEFT);
		
		$acc = substr($strdigkey, 4, 1);
		$di1 = substr($strdigkey, 5, 1);
		$di2 = substr($strdigkey, 6, 1);
		$di3 = substr($strdigkey, 7, 1);
		
		$params = array();
		$params["gps"] = @$_POST['signal_gps'];
		$params["gsm"] = 5;
		$params["odor"] = @$_POST['odor'];
		$params["mainpower"] = "0";
		$params["acc"] = $acc;
		$params["di1"] = $di1;
		$params["di2"] = $di2;
		...
		if($f1 != "")
		{
			$f1 = number_format(($f1 * 100) / 4095, 2);
		}
		$params["fuel1"] = $f1;
