	...
	*/
	...
	if(@$_POST['method'] == 'amwellInsertNew')
	{
		...
		*/
		...
		$loc['params'] =@$_POST['params']."gpsL=".@$_POST['signal_gps']."|"."acc=".@$_POST['acc']."|"."charge=".@$_POST['charge']."|"."oil_cnct=".@$_POST['oil_cnct']."|"."adc1=".@$_POST['analog_ac']."|"."di1=".@$_POST['ac']."|"."analog=".@$_POST['analog']."|"."external_power=".@$_POST['external_power']."|";
	...
	function insert_db_loc($loc)
	{
		...
		if (($loc['lat'] != 0) && ($loc['lng'] != 0))
		{
			...
			{
				//check_events($loc, true, true, false);
				if($loc['params']['adc1']==1){
					check_events($loc, true, true, true);
				}else{
					check_events($loc, true, true, false);
				}
			}
	...
	function insert_db_status($loc, $loc_prev)
	{
		...
		if (strtotime($loc['dt_tracker']) >= strtotime($loc_prev['dt_tracker']))
		{
			...
			if ($dt_last_stop >= $dt_last_move )
			{
				$sensor = getSensorFromType($imei, 'acc');
				$acc = $sensor[0]['param'];
				$acsensor = getSensorFromType($imei, 'air_ac');
				$ac=$acsensor[0]['param'];
