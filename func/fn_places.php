<? 
	...
	if(@$_POST['cmd'] == 'save_marker')
	{
		...
		if ($marker_id == 'false' && checkSettingsPrivileges('markers','add')==true)
		{
		...
		else if ($marker_id != 'false' && checkSettingsPrivileges('markers','edit')==true)
		{
	...
	{
		if(checkSettingsPrivileges('markers','delete')!=true){
			echo 'NO_PERMISSION';
			die;
		}
	...
	{
		if(checkSettingsPrivileges('zones','delete')!=true){
			echo 'NO_PERMISSION';
			die;
		}
	...
	if(@$_POST['cmd'] == 'save_zone')
	{
		...
		if ($zone_id == 'false')
		{
			if(checkSettingsPrivileges('zones','add')!=true){
				echo 'NO_PERMISSION';
				die;
			}	
		...
		{
			if(checkSettingsPrivileges('zones','edit')!=true){
				echo 'NO_PERMISSION';
				die;
			}
	...
	{
		if(checkSettingsPrivileges('route','delete')!=true){
			echo 'NO_PERMISSION';
			die;
		}
		...
		writeLog('user_access', 'Delete Route: '.$route_id);
	...
	if(@$_POST['cmd'] == 'delete_all_routes')
	{
		...
		writeLog('user_access', 'Delete all Route: '.$user_id);
	...
	if(@$_POST['cmd'] == 'save_route')
	{
		...
		if ($route_id == 'false')
		{
			if(checkSettingsPrivileges('route','add')!=true){
				echo 'NO_PERMISSION';
				die;
			}
		...
		{
			if(checkSettingsPrivileges('route','edit')!=true){
				echo 'NO_PERMISSION';
				die;
			}
	...
	if(@$_POST['cmd'] == 'load_zone_data')
	{
		...
		// check privileges
		if ($_SESSION["privileges"] == 'subuser' && $user_id!=726)
		{			
	...
	{ 
		...
		if ($r)
		{
			...
			while($row = mysqli_fetch_array($r))
			{
				...
				$modify='';
				if(checkSettingsPrivileges('markers','edit')==true){
