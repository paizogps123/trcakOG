<? 
	...
	{
		$q = "SELECT * FROM `gs_user_reports` WHERE `user_id`='".$user_id."' and rpt_type='G' ORDER BY `report_id` ASC";
		$r = mysqli_query($ms, $q);
	...
	{			
		...
		// get records number
		$q = "SELECT * FROM `gs_user_reports` WHERE `user_id`='".$user_id."'  and rpt_type='G' ";
		$r = mysqli_query($ms, $q);
		$count = mysqli_num_rows($r);
		
		$q = "SELECT * FROM `gs_user_reports` WHERE `user_id`='".$user_id."'  and rpt_type='G'  ORDER BY $sidx $sord";
		$r = mysqli_query($ms, $q);
		...
		if ($r)
		{
			...
			while($row = mysqli_fetch_array($r)) {
				...
				else if ($row['type'] == 'sensor_graph')
				{
			 	...
			 	else if ($row['type'] == 'maintenancereport')
				{
					$type = $la['MAINTENANCE'];
				}
				else
	...
	if(@$_POST['cmd'] == 'delete_report_generated')
	{
		...
		$q = "SELECT * FROM `gs_user_reports_generated` WHERE `report_id`='".$report_id."'  and rpt_type='G' ";
		$r = mysqli_query($ms, $q);
	...
	if(@$_POST['cmd'] == 'delete_selected_reports_generated')
	{
		...
		for ($i = 0; $i < count($items); ++$i)
		{
			...
			$q = "SELECT * FROM `gs_user_reports_generated` WHERE `report_id`='".$item."'  and rpt_type='G' ";
			$r = mysqli_query($ms, $q);
			$row = mysqli_fetch_array($r);
	...
	if(@$_GET['cmd'] == 'load_reports_generated_list')
	{
		...
		// get records number
		$q = "SELECT * FROM `gs_user_reports_generated` WHERE `user_id`='".$user_id."'  and rpt_type='G' ";
		$r = mysqli_query($ms, $q);
		$count = mysqli_num_rows($r);
		...
		$q = "SELECT * FROM `gs_user_reports_generated` WHERE `user_id`='".$user_id."'  and rpt_type='G'  ORDER BY $sidx $sord LIMIT $start, $limit";
		$r = mysqli_query($ms, $q);
