...
date_default_timezone_set("Asia/Kolkata");
    ...
    else if(@$_GET['cmd'] == 'select_allocate')
    {
        ...
        if ($result!=false)
        {
            ...
            else
            {
                ...
                $modify = '<a href="#" onclick="boardingallocatedit(\''.$row['event_id'].'\');" title="Edit"><img src="theme/images/edit.svg"></a><a href="#" onclick="boardingallocatedelete(\''.$row['event_id'].'\');" title="'.$la['DELETE'].'"><img src="theme/images/remove3.svg" /></a>';
	...
	if(@$_POST['cmd'] == 'save_hotspot')
	{
		...
		if ($hotspot_code == '0')
		{
			$q = "INSERT INTO `droute` (routename,user_id) VALUES ('".$hotspot_name."','".$user_id."')";
			$r = mysqli_query($ms,$q);
		...
		{
			$q = "UPDATE `droute`  SET  `routename`='".$hotspot_name."'  WHERE	`route_id`='".$hotspot_code."' and user_id='".$user_id."'";
			$r = mysqli_query($ms,$q);
	...
	else if(@$_POST['cmd'] == 'select_hotspot_single')
	{
		...
		$q = "select d.routename,dr.*,guz.zone_name from droute d join droute_sub dr on 
		...
		if ($result!=false)
		{
			...
			{
				$ir=$ir+1;
				$r1[] = array('routename' => $row['routename'],'sino'=>$ir,'zonecode'=>$row['zonecode'],'zonename'=>$row['zone_name'],'zoneinout' => $row['zoneinout'],'message' => $row['message'],'point' => $row['point']);
    ...
    if(@$_POST['cmd'] == 'save_student')
    {
        ...
        if(mysqli_num_rows($r) == 0 )
        {
     		  ...
     		  if ($uid == '0')
        	 {
		        $q = "INSERT INTO `dstudent` (dob,route_id_down,dept_id,section_id,sid,name,gender,route_id,status,user_id,rfid,create_date,zone_id,zone_id_down) VALUES ('".$dob."','".$route_id_down."','".$dept_id."','".$section_id."',
	            '".$sid."','".$name."','".$gender."','".$route_id."','".$status."','".$user_id."','".$rfidno."','".date("Y-m-d H:i:s")."','".$zone_id."','".$zone_id_down."')";
		        $r = mysqli_query($ms,$q);
        	 ...
        	 else
        	 {
           		...
           		rfid='".$rfidno."',zone_id='".$zone_id."',zone_id_down='".$zone_id_down."' WHERE `uid`='".$uid."' and user_id='".$user_id."'";
           		$r = mysqli_query($ms,$q);
...
,dsv.parent_name parent, dsv.contactno phno, dsv.emailid,s.rfid,s.zone_id,s.zone_id_down
from dstudent s
  ...
  s.section_id=ds.section_id
        ...
        if ($result!=false)
        {
            ...
            {    
                ...
                $responce->rows[$i]['id'] = $i;
                $responce->rows[$i]['cell']=array($row['dept_name'],$row['section_name'],$row['sid'],$row['name'],$row['gender'],$row['phno'],$row['dob'],$row['emailid'],$row['routename'],$row['routename_down'],$row['status'], $modify,$row['uid'],$row['route_id'],$row['section_id'],$row['dept_id'],$row['route_id_down'],$row['parent'],$row['rfid'],$row['zone_id'],$row['zone_id_down']);
                $i++;
            }
        }
