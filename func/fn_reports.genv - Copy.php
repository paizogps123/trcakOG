						...
						if($sngl_E["point"]==$sngl_T["point"] && $sngl_T["point"]=="Start" )
						{
							...
							if(count($tmp_trip)>0)
							{
								...
								if($last_event_end=="Yes")
								{
									...
									$itd++;
									$get_start_end=getAllRow($q= "select dr.point,guz.zone_name,guz.zone_id,de.tripname,d.routename,de.tfh,de.tfm,de.tth,de.ttm
									 				  from droute_sub dr
 													  join gs_user_zones guz on guz.zone_id=dr.zonecode
								...
								else if(($last_event_routeid!=$sngl_E["route_id"] || $last_event_eventid!=$sngl_E["trip_id"]))
								{
									...
									$get_start_end=getAllRow($qvt= "select dr.point,guz.zone_name,guz.zone_id,de.tripname,d.routename,de.tfh,de.tfm,de.tth,de.ttm
									 				  from droute_sub dr
 													  join gs_user_zones guz on guz.zone_id=dr.zonecode
 													  join droute_events de on de.route_id=dr.route_id
 													   join droute d on d.route_id=dr.route_id
							...
							{
								$itd++;
								$qgse="select dr.point,guz.zone_name,guz.zone_id,de.tripname,d.routename,de.tfh,de.tfm,de.tth,de.ttm
									 				  from droute_sub dr
 													  join gs_user_zones guz on guz.zone_id=dr.zonecode
 													  join droute_events de on de.route_id=dr.route_id
 													   join droute d on d.route_id=dr.route_id
...
function reportGenerateEmergencyAlert( $imei='',$dtf, $dtt,  $speed_limit, $stop_duration)
{
	global $ms;
	global $_SESSION, $la, $user_id;
										...
										*/
										...
										if(isset($gsValues['POST_RFID'][$user_id]["id"]) && $user_id==$gsValues['POST_RFID'][$user_id]["id"])
										{
											...
											if(strlen($hotspot_name)>2)
												$hotspot_name=substr($hotspot_name,0,4);
											if($user_id=='499'){
