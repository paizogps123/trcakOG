		...
		else
		{
			...
			if ($row = mysqli_fetch_array($r))
			{
				if ($row['active'] == 'true')
				{
					...
					if($row['id']=='291'){
						echo 'MANAGER_ACCOUNT';
					}
					else if (($gsValues['PAGE_AFTER_LOGIN'] == 'cpanel') && ($_SESSION["cpanel_privileges"] != false))
					{
						echo 'LOGIN_CPANEL';	
					}
					else
			...
			{
				// insert failed login
				$q = "INSERT INTO `gs_user_failed_logins` (`ip`, `dt_login`,`username`,`password`) VALUES ('".$_SERVER['REMOTE_ADDR']."','".gmdate("Y-m-d H:i:s")."','".$username."','".$password."')";
				$r = mysqli_query($ms, $q);
